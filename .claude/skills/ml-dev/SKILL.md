...
| Run W&B "assente" dopo delete+rilancio con lo stesso nome | Gli id cancellati sono **tombstonati per sempre** (`was previously created and deleted; try a new run id`); inoltre la delete è asincrona e può inghiottire in race una run appena ricreata con lo stesso id. Fix: dopo una delete si rilancia SEMPRE con `--run-name` diverso (suffisso `b`) |
| Traceback dei crash irrecuperabile dai log | Il lancio filtrava con `grep val_loss\|Traceback`: resta solo l'intestazione, il corpo dell'eccezione muore nel pipe. Log SEMPRE integrale su file (`> x.log 2>&1`); filtrare solo in lettura (`grep -v "it/s"`) |
